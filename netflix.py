...
current_user_option = user_options()
counter = 0
user =[]
passw =[]
updated_list=[]
...
while Loop_Options:
	...
	if current_user_option == "2":
	
		#Countries List in File
		
		if split_method_has_a_country:
			title()
			combolist_countries = {}
			counter = 0
			lines_counter = 0
			...
			print(countries[full_countries_list_from_dict[1]])
			while counter < len(full_countries_list_from_dict):
				...
				lines_counter = 0
				counter += 1
			print(combolist_countries)
			input("\n\nPress Enter to return")
			break
		else:
			input("No countries to sort. Press Enter to return")
			title()
			
	if current_user_option == "3":
